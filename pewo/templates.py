...
def get_experiment_dir_template(config: Dict, software: PlacementSoftware, **kwargs) -> str:
    ...
    if software == PlacementSoftware.EPA:
        ...
        if heuristic == "h1":
            return os.path.join(software_dir, input_set_dir_template, "h1", "g{g}", "mem{mem}",
                                get_common_queryname_template(config))
        elif heuristic == "h2":
            return os.path.join(software_dir, input_set_dir_template, "h2", "bigg{bigg}", "mem{mem}",
                                get_common_queryname_template(config))
        elif heuristic in ("h3", "h4"):
            return os.path.join(software_dir, input_set_dir_template, heuristic, "mem{mem}",
                                get_common_queryname_template(config))
...
def get_queryname_template(config: Dict, software: PlacementSoftware, **kwargs) -> str:
    ...
    if software == PlacementSoftware.EPA:
        ...
        if heuristic == "h1":
            return get_common_queryname_template(config) + "_h1_g{g}_mem_{mem}"
        elif heuristic == "h2":
            return get_common_queryname_template(config) + "_h2_bigg{bigg}_mem_{mem}"
        elif heuristic in ("h3", "h4"):
            return get_common_queryname_template(config) + "_" + heuristic + "_mem_{mem}"
    elif software == PlacementSoftware.PPLACER:
...
def get_output_template_args(config: Dict, software: PlacementSoftware, **kwargs) -> Dict[str, Any]:
    ...
    if software == PlacementSoftware.EPA:
        ...
        template_args["mem"] = config["config_epang"]["maxmem"]
    elif software == PlacementSoftware.PPLACER:
